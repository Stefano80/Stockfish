...
Value Eval::evaluate(const Position& pos, Lazy lazy) {
  ...
  if (lazy == LAZY_FORCED || (lazy == LAZY && abs(v) > LazyThreshold))
...
template Value Eval::evaluate<true >(const Position&, Lazy lazy);
template Value Eval::evaluate<false>(const Position&, Lazy lazy);
...
std::string Eval::trace(const Position& pos) {
  ...
  Value v = evaluate<true>(pos, LAZY_NONE);
