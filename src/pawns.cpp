...
namespace {
  ...
  Score evaluate(const Position& pos, Pawns::Entry* e) {
    ...
    e->passedPawns[Us] = e->pawnAttacksSpan[Us] = e->weakUnopposed[Us] = e->pawnSpace[Us] = 0;
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        e->pawnSpace[Us]       |= forward_file_bb(Them, s);
...
Entry* probe(const Position& pos) {
  ...
  e->score += make_score(popcount(e->pawnSpace[WHITE] && ~e->pawnSpace[BLACK]), 0);
  e->score -= make_score(popcount(e->pawnSpace[BLACK] && ~e->pawnSpace[WHITE]), 0);
